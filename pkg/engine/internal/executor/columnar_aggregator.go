...
type columnarAggregatorOpts struct {
	...
	matchWindows  columnarWindowMatchFunc // nil = identity mode
...
type columnarAggregator struct {
	...
	updateFunc func(values []float64, counts []int64, groupIdxForRow []int, inputValues []float64)
	...
	hashBuffer []uint64 // per-row series hashes
	outTSBuf   []int64  // per-row resolved output timestamps
...
func newColumnarAggregator(pointsSizeHint int, opts columnarAggregatorOpts) *columnarAggregator {
	...
	a.updateFunc = a.resolveUpdateFunc(opts.operation)
	return a
}
...
func (a *columnarAggregator) AddBatch(
	...
	// TODO:
	// 1. For grouping without labels, we can skip hash computation,
	//    and directly update a single aggregator.
	// 2. This does not consider collisions where multiple series have
...
// offset buffers, avoiding per-row Value() overhead
...
// combineHashes combines two 64-bit hash values using the boost::hash_combine
// technique.
// See https://www.boost.org/doc/libs/release/libs/container_hash/doc/html/hash.html#notes
func combineHashes(seed, value uint64) uint64 {
...
func (a *columnarAggregator) resolveWindows(timestamps *array.Timestamp) []int64 {
	...
	a.opts.matchWindows(a.outTSBuf, a.outTSBuf, a.validMask, numRows)
	return a.outTSBuf
}
